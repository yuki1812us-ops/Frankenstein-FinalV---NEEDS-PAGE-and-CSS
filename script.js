  ...
  var PercyArray = Array.from(visible_percy);
    ...
    //write an forEach() method that shows all the text written and modified by both hand (in black?). The forEach() method of Array instances executes a provided function once for each array element.
      MaryArray.forEach((item) => {
            item.style.color = '#060602ff';
            item.style.backgroundColor = 'transparent'; // Clean up highlights
        });
     ...
     //write an forEach() method that shows all the text written and modified by Mary in a different color (or highlight it) and the text by Percy in black. 
      MaryArray.forEach((item) => {
            item.style.backgroundColor = '#ffff00ff';
            item.style.color = '#020112e5'; 
        });
      PercyArray.forEach((item) => {
        item.style.color = '#020206ff';
     ...
     //write an forEach() method that shows all the text written and modified by Percy in a different color (or highlight it) and the text by Mary in black.
      PercyArray.forEach((item) => {
            item.style.color = '#200defff';
            item.style.backgroundColor = '#14e126ff';
        });
      MaryArray.forEach((item) => {
        item.style.color = '#02011dff';
        item.style.backgroundColor = 'transparent';
    });
    }
  }
...
function toggleDeletions() {
    ...
    if(textDiv){
      textDiv.classList.remove("reading-mode"); 
      textDiv.classList.toggle("hide-del");
}
}
...
(function setupProjectControls() {
    ...
    if (controls) {
        ...
        if (!document.getElementById("btn-del")) {
            ...
            btnDel.id = "btn-del";
            btnDel.innerHTML = "Deletions";
            btnDel.className = "btn btn-secondary mt-2 mr-2";
            btnDel.onclick = toggleDeletions;
            controls.appendChild(btnDel);

            var btnRead = document.createElement("button");
            btnRead.id = "btn-read";
            btnRead.innerHTML = "Reading Mode";
            btnRead.className = "btn btn-primary mt-2";
            btnRead.onclick = toggleReadingMode;
